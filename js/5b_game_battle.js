...
function renderQuestion() {
    ...
    boss.classList.remove("dragon-attack", "boss-hit-impact");
    boss.style.filter = ""; 
...
function checkAnswer(userAns, btnElement) {
    ...
    if (isCorrect) {
        ...
        if (typeof playPlayerHitVFX === 'function') {
            const bossRect = document.getElementById("bossImage").getBoundingClientRect();
            const boxRect = box.getBoundingClientRect();
            const startX = boxRect.left + boxRect.width / 2;
            const startY = boxRect.top + Math.min(38, boxRect.height * 0.28);
            const endX = bossRect.left + bossRect.width / 2;
            const endY = bossRect.top + bossRect.height * 0.48;
            playPlayerHitVFX(startX, startY, endX, endY);
        }
        ...
        if (typeof playDragonAttackVFX === 'function') {
            playDragonAttackVFX(startX, startY, endX, endY);
        } else {
